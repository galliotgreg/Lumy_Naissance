...
public class DragDropScript : MonoBehaviour {
	...
	void Update () {
        ...
        if(isMouseDragging)
        {
            ...
            //tracking mouse pos
            Vector3 currentScreenSpace = new Vector3(Input.mousePosition.x, Input.mousePosition.y, -GameObject.Find("Main Camera").GetComponent<Camera>().transform.position.z);

            //converting screen pos => world pos with offset.
            Vector3 currentPosition = GameObject.Find("Main Camera").GetComponent<Camera>().ScreenToWorldPoint(currentScreenSpace); // + offsetValue;
