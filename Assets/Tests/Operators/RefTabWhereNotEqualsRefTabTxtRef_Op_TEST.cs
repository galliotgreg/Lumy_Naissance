...
using UnityEngine;
class RefTabWhereNotEqualsRefTabTxtRef_Op_TEST : MonoBehaviour {

    [SerializeField]
    private string symbol = "R[]where!=R[]TR";
    [SerializeField]
    ...
    private void RandomizeTest() {
        ...
        AB_RefTab_WhereNotEquals_RefTab_Txt_Ref_Operator ope = (AB_RefTab_WhereNotEquals_RefTab_Txt_Ref_Operator)ABOperatorFactory.CreateOperator(symbol);
        ...
        for (int i = 0; i < tabRef1.Value.Values.Length; i++) {
            if (tabRef1.Value.Values[i].GetAttr(r2) != null && tabRef1.Value.Values[i].GetAttr(arg2.Value.Value) != ref1.Value) {
